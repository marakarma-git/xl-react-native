...
const dataUsageSubscribersAnalytics = [
  {
...
const initialState = {
  dataHeader: dataUsageSubscribersAnalytics,
  generatedParams: '',
  appliedFilter: [],
...
const usage_subscribers_analytics_filter_reducer = (
  state = initialState,
  action,
) => {
  switch (action.type) {
    case reduxString.USAGE_SUBSCRIBERS_ANALYTICS_DYNAMIC_ONCHANGE_DROP_DOWN: {
      const index =
    ...
    case reduxString.USAGE_SUBSCRIBERS_ANALYTICS_DYNAMIC_SUCCESS: {
      const index =
        state.dataHeader.findIndex((f) => f.formId === action.formId) || 0;
    ...
    case reduxString.USAGE_SUBSCRIBERS_ANALYTICS_DYNAMIC_FAILED: {
      const index =
        state.dataHeader.findIndex((f) => f.formId === action.formId) || 0;
    ...
    case reduxString.USAGE_SUBSCRIBERS_ANALYTICS_DYNAMIC_LOADING: {
      const index =
        state.dataHeader.findIndex((f) => f.formId === action.formId) || 0;
      state.dataHeader[index].disabled = true;
    ...
    case reduxString.USAGE_SUBSCRIBERS_ANALYTICS_DYNAMIC_RESET_SELECTED_VALUE: {
      const index =
        state.dataHeader.findIndex((f) => f.formId === action.formId) || 0;
      state.dataHeader[index].value = {};
      if (
    ...
    case reduxString.USAGE_SUBSCRIBERS_ANALYTICS_RESET_ALL_VALUE: {
    ...
    case reduxString.USAGE_SUBSCRIBERS_ANALYTICS_GENERATE_PARAMS: {
    ...
    case reduxString.USAGE_SUBSCRIBERS_ANALYTICS_RESET_GENERATE_PARAMS: {
      return {
        ...state,
    ...
    case reduxString.USAGE_SUBSCRIBERS_ANALYTICS_SET_PARAMS_NAVIGATION: {
      return {
        ...state,
        appliedFilterForParams: action.appliedFilterParams,
      };
    }
    case reduxString.USAGE_SUBSCRIBERS_ANALYTICS_RESET_PARAMS_NAVIGATION: {
      return {
        ...state,
...
export default usage_subscribers_analytics_filter_reducer;
