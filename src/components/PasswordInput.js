...
const passwordFormArray = [
    { name: 'oldPassword', label: 'Current Password', required: true, visible: false, validation: false },
    { name: 'newPassword', label: 'New Password', required: true, visible: false, validation: true },
    { name: 'confirmPassword', label: 'Confirm Password', required: true, visible: false, validation: false }
];
...
const PasswordInput = ({ submitHandler, requestLoading, navigation }) => {
    ...
    const passwordValidator = (value) => {
        ...
        if (value.length > 0) {
            ...
            newPasswordRules[5].valid = /^[a-zA-Z0-9#*!?+&@.$%\-,():;/]+$/.test(value) ? true : false;
        ...
        passwordForm.map((forms, index) => (
            <View key={index} style={styles.formGroup}>
                ...
                <View style={styles.passwordInputWrapper}>
                    <TextInput
                        onFocus={() => forms.validation && passwordValidator(form.newPassword)}
                        onChangeText={(text) => inputHandler(forms.name, text, forms.validation)}
        ...
        <View style={[styles.formContainer, { marginTop: 10 }]}>
            ...
            <View style={styles.buttonGroupContainer}>
                ...
                <TouchableOpacity
                    ...
                    style={[styles.buttonGroup, { backgroundColor: !formComplete ? '#949494' : '#002DBB' }]}>
                    {
                        requestLoading
